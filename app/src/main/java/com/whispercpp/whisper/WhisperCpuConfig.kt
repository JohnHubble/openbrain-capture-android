...
object WhisperCpuConfig {
    /**
     * Thread count for whisper.cpp. The frequency-binning heuristic in [CpuInfo] drops all
     * cores at the lowest frequency — on Samsung/Qualcomm big.BIG SoCs (e.g. Snapdragon 8
     * Elite: 2 prime + 6 performance) that discards every performance core and leaves only
     * the 2 prime cores, a 3–4× throughput loss. Coerce upward to at least
     * (availableProcessors − 2), reserving a pair of cores for the audio thread and UI.
     */
    val preferredThreadCount: Int
        get() = CpuInfo.getHighPerfCpuCount()
            .coerceAtLeast((Runtime.getRuntime().availableProcessors() - 2).coerceAtLeast(4))
}
